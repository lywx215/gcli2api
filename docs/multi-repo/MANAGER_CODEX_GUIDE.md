...
方案修订：`manager-architecture-1.4`（2026-08-30）
...
当前项目由唯一仓库所有者单人开发；所有者在任务、Issue或PR中明确批准具体提案即可满足
人工Review，不要求额外GitHub reviewer。Codex和CI均不能自行代替所有者批准，批准也不
解除双仓文档哈希、测试、安全、handoff、灰度和回滚要求。
...
| 阶段 | 工作项 | 核心交付 |
|---|---|---|
| M1 | MGMT-001 | FastAPI、MySQL、单管理员、React基础骨架，已完成 |
| M2 | MGMT-002 | 节点注册、密钥加密、安全探测和适配器骨架 |
| M3 | MGMT-003 | Legacy只读摘要、凭证、统计和页面 |
| M4 | MGMT-004 | Modern V1只读Management API及兼容矩阵 |
...
| M8.5 | MGMT-011 | Pro共享算力预算、北京时间24小时恢复预测和手动额度刷新 |
| M8.6 | MGMT-012 | 节点控制台、`#manage`深链接、受控iframe和新标签回退 |
| M9 | MGMT-009 | 20台支持矩阵及2台、5台、剩余节点灰度 |
| M10 | MGMT-010 | 正式发布、监控告警、Runbook和持续维护 |

每一项的详细范围、排除项、跨仓顺序、用户输入和验收以
`IMPLEMENTATION_ROADMAP.md`为准。不得跳过中间工作项，也不得在当前工作项中提前实现
后续能力。
...
```text
左侧导航：总览 / 节点 / 节点控制台 / 算力预算 / 凭证 / 任务 / 审计日志 / 设置
...
- 算力预算使用Pro共享额度桶，展示总算力、额度覆盖和北京时间未来24小时恢复分布；
- 预算页额度刷新必须由管理员手动确认，复用主动操作缓存、并发、限流和任务进度；
- 耗时任务显示进度、逐项结果和安全重试入口；
- 未知字段显示`—`，不得显示为0；
- 节点离线时展示最后快照并禁用写操作；
- 不支持的capability必须禁用或隐藏操作，并向用户说明原因。
- 节点控制台列出全部节点；停用节点禁止打开，离线或缺少嵌入能力的节点默认提供新标签，
  并可由唯一管理员显式点击“尝试内嵌”进入持续标注未验证的诊断iframe；
- iframe只加载manager API返回的已注册HTTPS节点入口，必须验证版本化握手并始终提供
  “在新标签打开”回退，不传递manager或节点认证信息。诊断iframe复用同一沙箱，但不得
  因加载成功或收到消息改变capability、支持等级或G6.6状态；

### 5.3 前端代码约束

- `ProTable`负责节点、凭证、任务和审计等主要列表；
- 前端只能调用生成的manager API Client，不得直接调用gcli2api业务API；MGMT-012唯一
  例外是把manager API返回的已校验节点控制面板URL作为顶层导航或隔离iframe加载，前端
  不得读取其DOM、调用其函数、代理内容或附加认证信息；
...
- Preview只有启用能力时不展示关闭；
- 额度任务不会泄漏Token；
- Enterprise/Standard的500/250规则、66%换算、共享Pro桶、模型冲突和北京时间跨日正确；
- 预算页在无额度能力、快照陈旧、未知Tier和部分刷新失败时不制造0或自动调用Google；
- 节点离线不影响其他节点页面；
...
- 未实现任何new-api读取或写入。
- `/#manage`深链接、Origin白名单、CSP、握手来源校验、停用/离线/旧节点回退和浏览器
  存储受限场景均通过，URL与跨窗口消息不含敏感字段；
