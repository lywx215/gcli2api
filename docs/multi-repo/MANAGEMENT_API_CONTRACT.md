...
嵌入；默认使用新标签打开已注册节点的HTTPS控制面板。唯一管理员可在回退界面显式点击
一次“尝试内嵌”执行未验证诊断：只允许使用manager服务端已经构造的同一HTTPS入口，复用
正式iframe的沙箱和`no-referrer`，持续显示“未验证”，并保留新标签入口。诊断模式不要求
握手成功后才保留页面，但不得读取DOM、传递认证、把加载成功或收到消息记为capability，
也不得作为G6.6通过证据。停用节点禁止打开；离线节点的诊断尝试同样必须由管理员显式点击。
