...
桌面端App是定位、任务、数据库和发布流程均独立的产品，不是manager的客户端或数据源。
`SUBS-LINK-20260909`是单独审核的有限例外：桌面端只可作为普通HTTPS客户端调用节点公开
Management API，仍不得共享数据库、源码、账号、任务或发布流程，也不得直接读取节点存储。
...
| Management schema | `1.4` | HTTP协议兼容判断 |
...
本项目当前采用单人所有者Review：不要求额外GitHub reviewer；仓库所有者在Codex任务、
PR或Issue中对具体提案作出明确批准，即满足该提案的Review要求。CI通过不能替代所有者
批准，Codex也不得自行批准。批准只解除人工Review条件，不解除共享文档哈希、依赖、测试、
handoff、灰度、安全和回滚门禁。
...
- 对未知节点的降级行为。
- 节点控制台深链接、允许嵌入Origin、握手版本和旧节点回退行为（如适用）。
...
- 管理接口只返回元数据；
- 节点控制台URL、fragment和跨窗口消息不得携带面板密码、Management Token或凭证内容；
- 对节点请求禁止跨域名携带认证重定向；
- 删除和永久禁用必须二次确认并审计；
