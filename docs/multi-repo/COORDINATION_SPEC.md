...
也不得把该例外扩展为前端直接调用节点业务API。自动且标记为“已验证”的内嵌必须由
`ui.credential_console.embed` capability、精确Origin白名单和握手共同决定；缺失任一条件
时默认安全回退，不得按版本猜测。唯一管理员可在回退界面显式点击“尝试内嵌”进行一次
未验证的诊断试载：只能复用服务端构造的HTTPS入口和相同iframe沙箱，必须持续标注未验证、
保留新标签入口，并且不得把加载成功、页面外观或收到消息提升为capability或G6.6证据。
