...
路线图版本：`implementation-roadmap-1.3`（2026-08-30）
...
状态：`in_progress`。双仓正式实现、gcli2api固定候选镜像和manager自动化G6.6矩阵均已
完成；所有者另行批准了不改变capability的显式诊断iframe试载。`v109`和`p01`已选为两台
非关键节点，但仍需正式manager HTTPS Origin和两节点候选部署证据后，MGMT-012才能标记
为`done`，MGMT-009才能重新标记为`ready`。
...
- 仅对已启用、在线、Modern V1且声明`ui.credential_console.embed`的节点尝试iframe；
  停用节点禁止打开，离线及缺失能力节点默认只允许新标签；唯一管理员可在回退界面显式
  点击一次“尝试内嵌”诊断试载，但不得据此推断能力或改变节点支持等级；
...
- 只接受来源Origin和`event.source`均与所选节点匹配的版本化`postMessage`握手；超时后
  终止已验证嵌入并展示新标签入口，新标签始终作为显式回退可用。诊断试载不声称已建立
  安全握手，超时后保留iframe供管理员观察并持续显示未验证警告；浏览器拒绝加载时由用户
  使用新标签回退；
...
或Unknown猜测嵌入能力、把诊断试载当成正式支持，以及按版本字符串推断支持。

验收：未登录、已登录和刷新后`/#manage`均保持目标tab；错误hash安全回退；错误Origin、
伪造消息、HTTP URL、跨域重定向和未授权祖先被拒绝；停用、离线、能力缺失、握手超时及
浏览器存储受限均有明确回退；显式诊断试载只接受服务端HTTPS入口、保持沙箱、持续显示
未验证状态且不改变capability；数据库、日志、URL和消息无敏感信息；manager先发布，随后
...
| 节点控制台 | 固定`/#manage`；独立登录；capability优先；回退页可由管理员显式执行未验证iframe诊断试载；不代理、不传递凭证 |
