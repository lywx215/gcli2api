...
- SQLite在单一`BEGIN IMMEDIATE`事务内校验状态令牌、凭证替换、禁用/永久禁用、身份和
  元数据完整性及所有`required_models`、`*`、`all`冷却，再执行启用；历史错误码、健康
  与隔离状态只供诊断，不作为启用前置条件；
...
并发条件启用、缺失字段、空/401/403/429/混合或非法错误码、检查/风险/人工隔离诊断、永久
禁用、目标及通配模型冷却、API禁用和测试200/429；Legacy`/creds/*`与原Management请求保持兼容；
OpenAPI与本契约一致。
