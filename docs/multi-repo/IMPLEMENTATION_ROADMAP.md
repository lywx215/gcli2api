...
| G6 生产门禁 | TLS、权限、备份恢复、监控和安全Review通过 | MGMT-009 |
...
| MGMT-008 | 腾讯云数据库、Zeabur配置、备份恢复和安全验收 | manager/运维 | MGMT-007 | `planned` |
...
- 使用`MANAGER_MASTER_KEY`信封加密节点Token或Legacy凭证，浏览器永远不能回读明文；
...
- 创建唯一manager数据库账号，权限仅为`gcli2api_manager.*`上的读写，不得出现`*.*`
  或`GRANT OPTION`；
- 配置TLS CA校验、实际网络来源限制、自动备份和时间点恢复；
- 在Zeabur Secret中配置数据库、管理员哈希、会话密钥和主密钥；
- 明确主密钥和数据库备份的独立保管及轮换方式。

Codex范围：提供脱敏配置示例、启动前校验、`SHOW GRANTS`检查、迁移/恢复Runbook、健康
检查、日志脱敏和监控规则。未经明确授权不得连接或修改生产数据库。

验收：空库迁移、上一版本升级、备份恢复演练、权限验证、TLS验证、密钥扫描、管理员会话
和登录限流全部通过。
...
| 数据库 | `utf8mb4`、优先`utf8mb4_0900_ai_ci`、单一库级读写账号、TLS必需 |
...
5. 数据库、TLS、备份和Zeabur Secret已经配置完成的脱敏证明。
