...
def test_parallel_tool_calls_get_distinct_stream_indices():
    ...
    _STREAM_TOOL_INDEX.pop((response_id, 0), None)
...
def test_stream_index_is_isolated_per_response_id():
    _STREAM_TOOL_INDEX.pop(("resp-a", 0), None)
    _STREAM_TOOL_INDEX.pop(("resp-b", 0), None)
...
def test_stream_index_released_after_finish_reason():
    ...
    assert (response_id, 0) not in _STREAM_TOOL_INDEX
...
def test_stream_indices_are_isolated_per_candidate():
    response_id = "resp-multi-candidate"
    _STREAM_TOOL_INDEX.pop((response_id, 0), None)
    _STREAM_TOOL_INDEX.pop((response_id, 1), None)

    chunk = "data: " + json.dumps({"candidates": [
        {"index": 0, "content": {"role": "model", "parts": [
            {"functionCall": {"name": "first", "args": {}}}
        ]}},
        {"index": 1, "content": {"role": "model", "parts": [
            {"functionCall": {"name": "second", "args": {}}}
        ]}},
    ]})
    payload = json.loads(convert_gemini_to_openai_stream(
        chunk, "gemini-3.5-flash", response_id
    )[len("data: "):])

    assert payload["choices"][0]["delta"]["tool_calls"][0]["index"] == 0
    assert payload["choices"][1]["delta"]["tool_calls"][0]["index"] == 0
